"""Class-based views for the fazendaria module."""
...
class ProcessoListView(LoginRequiredMixin, ListView):
    template_name = "fazendaria/processo_list.html"
    ...
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        ...
        ctx["counters_list"] = [
            ...
            for value, label in SituacaoFazendaria.choices
        ]
        ctx["situacoes"] = SituacaoFazendaria.choices
...
class ProcessoDetailView(LoginRequiredMixin, DetailView):
    ...
    template_name = "fazendaria/processo_detail.html"
...
class ProcessoCreateView(LoginRequiredMixin, CreateView):
    model = ProcessoFazendaria
    form_class = ProcessoFazendariaForm
    template_name = "fazendaria/processo_form.html"
    success_url = reverse_lazy("fazendaria:lista")

    def form_valid(self, form):
        form.instance.criado_por = self.request.user
        response = super().form_valid(form)
        logger.info(
            "fazendaria process created",
            extra={
...
class ProcessoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self) -> str:
        return reverse_lazy("fazendaria:detalhe", kwargs={"pk": self.object.pk})

    def form_valid(self, form):
        response = super().form_valid(form)
        logger.info(
            "fazendaria process updated",
...
class ProcessoExportView(LoginRequiredMixin, View):
    ...
    def get(self, request: HttpRequest, formato: str) -> HttpResponse:
        ...
        filename = f"processos-fazendaria-{date.today():%Y-%m-%d}"
        if formato == "csv":
            return export_csv(qs, export_columns(), filename)
        if formato == "xlsx":
            return export_xlsx(
                qs, export_columns(), filename, sheet_title="Fazendária"
            )
